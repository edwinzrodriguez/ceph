				       ...
				       snapid_t cfollows)
{
  ...
  while (parent) {
    ...
    if (defer_inode_prop) {
      // Caller deferred parent scatter wrlocks; update the dirfrag fnode in
      // place and skip inode dirstat propagation until wrlocks are held.
      dout(10)
  ...
  for (unsigned depth = 0; depth < path.depth(); ) {
    ...
    */
	...
	return r;
      ...
      // the create/link reply.  Defer here; Server::ensure_parent_dir_wrlocks()
      // re-acquires after the request's own cap release so sibling clients
      // still lose Fs/COMPLETE.  MKDIR/MKNOD/SYMLINK still wrlock here.
      ...
      // caps).  Server::ensure_parent_dir_wrlocks() re-acquires later.
