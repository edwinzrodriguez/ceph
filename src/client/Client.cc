...
Client::C_Readahead::C_Readahead(Client *c, Fh *f, Inode *inode) :
    client(c), f(f), in(inode), start_time(ceph_clock_now()) {
  f->get();
...
void Client::C_Readahead::finish(int r) {
  ...
  client->client_finisher.queue(new LambdaContext(
    [client=client, in=in, fh, r](int) {
      ClientLockIfNeeded lock(client);
      lgeneric_subdout(client->cct, client, 20) << "client." << client->get_nodeid() << " C_Readahead on " << in << dendl;
			...
			Context *onfinish)
{
  ...
  {
    ceph::unique_unlock<ceph::TrackedLock> cl_drop(client_lock);
    r = objectcacher->file_read(oc.oset, &oc.layout, oc.snapid,
...
int Client::ll_read(Fh *fh, loff_t off, loff_t len, bufferlist *bl)
{
  ...
  /* We can't return bytes read larger than INT_MAX, clamp len to that */
