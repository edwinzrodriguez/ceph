  ...
  bufferlist& out)
{
  ...
  {
    std::scoped_lock<Client> l{*m_client};
...
Client::~Client()
{
  ceph_assert(ceph_mutex_is_not_locked(*this));
...
void Client::shutdown() 
{
  ...
  objectcacher->stop();  // outside of m_client_lock! this does a join.
				 ...
				 const UserPerm& request_perms)
{
  Inode *in;
  bool was_new = false;
  // inode_map and mount root state are protected by m_client_lock, not inode_lock.
  {
			 ...
			 size_t feature_needed)
{
  ...
  while (1) {
    ...
    // send request.
    // Release m_client_lock before calling send_request() to maintain proper lock ordering.
    // send_request() -> encode_cap_releases() -> encode_inode_release() needs inode_lock,
    // and the correct lock ordering is: inode_lock first, then m_client_lock.
    {
			 ...
			 int unless, int force)
{
  ...
  if (it != in->caps.end()) {
    ...
    // Caller must not hold m_client_lock to maintain lock ordering
    std::unique_lock in_lock(*in);
			   ...
			   mds_rank_t mds, int drop, int unless)
{
  ...
  if (dn->dir) {
    ...
    // when called from send_request which may hold m_client_lock.
...
void Client::handle_client_reply(const MConstRef<MClientReply>& reply)
{
  ...
  // insert_trace: that path drops m_client_lock and make_request may wake on
  ...
  if (signal_caller) {
	...
	return !request->dispatch_cond;
      ...
      // reply; do not wait on dispatch_cond or we pin m_client_lock until the
...
void Client::cancel_commands(const MDSMap& newmap)
{
  cancel_commands_if([=, this](MDSCommandOp const& op) {
    ...
    if (newmap.is_dne_gid(op_mds_gid) || newmap.is_laggy_gid(op_mds_gid)) {
       ...
       * m_client_lock here, because the con will
       * has its own lock.
       */
      op.con->mark_down();
      return -ETIMEDOUT;
...
void Client::delay_put_inodes(bool wakeup)
{
  ...
  // Drop m_client_lock for _put_inode (inode_lock only).  Use unique_unlock so we
...
void Client::put_inode(Inode *in, int n)
{
  ...
  // inode_lock only; mount_cond requires m_client_lock for a strict notify.
  // The unmount path batches delay_put_inodes() itself — do not drain here
  // while a std::unique_lock<Client> is still active on the stack.
  if (unmounting) {
...
void Client::_invalidate_inode_cache(Inode *in)
{
  ...
  if (cct->_conf->client_oc) {
    ...
    bool is_empty = objectcacher->set_is_empty(&in->oset);
...
bool Client::_flush_inode_async(Inode *in)
{
  return _flush(in, new C_Client_FlushComplete(this, in));
}
...
void Client::flush_set_callback(ObjectCacher::ObjectSet *oset)
{
  ...
  // Lock order is inode_lock then m_client_lock (_put_inode); _flushed needs
  // only inode_lock.  Reads may still hold m_client_lock via C_ReentrantLock on
  // the writeback finisher path — drop the full reentrant depth with scoped_drop.
...
{
  client_caps->add_update_cap(in, mds_session, cap_id, issued, wanted, seq, mseq, realm, flags, cap_perms);
}
...
public:
  ...
  void finish(int r) override {
    ...
    ceph_assert(ceph_mutex_is_not_locked_by_me(*client));
...
int Client::may_delete(const walk_dentry_result& wdr, const UserPerm& perms, bool check_perms)
{
  ...
  int r;
  if (in && in != diri) {
    std::unique_lock diri_lock(*diri, std::defer_lock);
    std::unique_lock in_lock(*in, std::defer_lock);
...
int Client::may_delete(const char *relpath, const UserPerm& perms) {
  ...
  if (int rc = path_walk(get_cwd_ref(), filepath(relpath), &wdr, perms, {.followsym = false}); rc < 0) {
    return rc;
  }
...
void Client::_close_sessions()
{
  ...
  while (!mds_sessions.empty()) {
    ...
    ldout(cct, 2) << "waiting for " << mds_ranks_closing.size() << " mds session(s) to close (timeout: "
                  << timo << "s)" << dendl;
    std::unique_lock<Client> l{*this, std::adopt_lock};
    if (!timo) {
      ceph::client_lock::wait_mount(*this, l);
    } else if (!ceph::client_lock::wait_mount_for(
                 *this, l, ceph::make_timespan(timo),
                 [this] { return mds_ranks_closing.empty(); })) {
...
void Client::_unmount(bool abort)
{
  ...
  while (lru.lru_get_size() > 0 ||
    ...
    // m_client_lock briefly to avoid ABBA with ms_dispatch.  Pin inodes first:
    // delay_put_inodes/trim_cache may delete map entries while m_client_lock
    // is dropped.
    {
...
relookup:
  ...
  if (dn) {
    ...
    bool has_caps = false;
    if (dn->inode) {
      std::unique_lock dn_lock(*(dn->inode));
		      ...
		      std::string trimmed_path)
{
  ...
  while (i < path.depth() && diri) {
    ...
    }
    {
      ...
      // _lookup/_do_lookup may take m_client_lock (make_request) then inode_lock.
      if (i == (path.depth() - 1)) {
...
int Client::do_rename(const char *relfrom, const char *relto, const UserPerm& perm, std::string alternate_name)
{
  ...
  return _rename(get_cwd_ref().get(), relfrom, cwd.get(), relto, perm, std::move(alternate_name));
}
                    ...
                    std::string alternate_name, FSCrypt_Options fscrypt_options)
{
  ...
  {
    std::scoped_lock<Client> lock(*this);
    int r = _get_fd_inode(dirfd, &dirinode);
    if (r < 0) {
      return r;
    }
}
...
int Client::mkdirs(const char *relpath, mode_t mode, const UserPerm& perms)
{
  ...
  for (;;) {
    walk_dentry_result wdr;
    if (int rc = path_walk(get_cwd_ref(), path, &wdr, perms, {.followsym = false}); rc < 0) {
      if (rc == -ENOENT) {
        InodeRef in;
...
int Client::mknod(const char *relpath, mode_t mode, const UserPerm& perms, dev_t rdev) 
{
  ...
  return _mknod(get_cwd_ref().get(), relpath, mode, rdev, perms);
}
		    ...
		    const UserPerm& perms)
{
  ...
  InodeRef in;
  if (int rc = path_walk(get_cwd_ref(), relpath, &in, perms, {}); rc < 0) {
    return rc;
  }
  std::unique_lock in_lock(*in);
		     ...
		     const UserPerm& perms, int flags)
{
  ...
  if (int rc = path_walk(get_cwd_ref(), relpath, &in, perms, {.followsym = !(flags & AT_SYMLINK_NOFOLLOW)}); rc < 0) {
    return rc;
  }
  std::unique_lock in_lock(*in);
  return _setattrx(in, stx, mask, perms);
		 ...
		 frag_info_t *dirstat, int mask)
{
  ...
  if (int rc = path_walk(get_cwd_ref(), relpath, &in, perms, {.mask = (unsigned)mask}); rc < 0) {
    return rc;
  }
  std::unique_lock in_lock(*in);
		  ...
		  const UserPerm& perms, frag_info_t *dirstat, int mask)
{
  ...
  if (int rc = path_walk(get_cwd_ref(), path, &in, perms, {.followsym = false, .mask = (unsigned)mask}); rc < 0) {
    return rc;
  }
  std::unique_lock in_lock(*in);
  if (int rc = _getattr(in, mask, perms); rc < 0) {
                   ...
                   const UserPerm& perms)
{
  ...
  if (int rc = path_walk(get_cwd_ref(), path, &in, perms, {}); rc < 0) {
    return rc;
  }
                    ...
                    const UserPerm& perms)
{
  ...
  if (int rc = path_walk(get_cwd_ref(), path, &in, perms, {.followsym = false}); rc < 0) {
    return rc;
  }
  struct ceph_statx attr;
  utime_t(times[0]).to_timespec(&attr.stx_atime);
...
int Client::opendir(const char *relpath, dir_result_t **dirpp, const UserPerm& perms)
{
  ...
  if (int rc = path_walk(get_cwd_ref(), filepath(relpath), &in, perms, {}); rc < 0) {
    return rc;
  }
  if (should_check_perms()) {
...
// common parts for open and openat. call with m_client_lock locked.
...
void Client::C_Read_Finisher::finish_io(int r)
{
  ...
  // Caller holds inode_lock so we don't need to take it.
	               ...
	               Context *onfinish, bool do_fsync, bool syncdataonly)
{
  ...
  if (cct->_conf->client_oc &&
    ...
    if (onfinish == nullptr) {
      ...
      //Register a wrapper callback C_Lock_Client_Finisher for the C_Write_Finisher which takes 'm_client_lock'.
      //Use C_OnFinisher for callbacks. The op_cancel_writes has to be called without 'm_client_lock' held because
      //the callback registered here needs to take it. This would cause incorrect lock order i.e., objecter->rwlock
      //taken by objecter's op_cancel and then 'm_client_lock' taken by callback. To fix the lock order, queue
...
void Client::C_nonblocking_fsync_state_advancer::finish(int r)
{
  ...
  ceph_assert(ceph_mutex_is_locked_by_me(*(state->in)));
  state->advance();
}
...
int Client::chdir(const char *relpath, const UserPerm& perms)
{
  ...
  std::unique_lock<Client> cock(*this);
  cwd = std::move(in);
...
int Client::statfs(const char *path, struct statvfs *stbuf, const UserPerm& perms)
{
  ...
  walk_dentry_result wdr;
  if (int rc = path_walk(get_cwd_ref(), filepath(path), &wdr, perms, {}); rc < 0) {
    return rc;
  }
                   ...
                   mode_t mode, const std::map<std::string, std::string> &metadata)
{
  ...
  walk_dentry_result wdr;
  if (int rc = path_walk(get_cwd_ref(), filepath(relpath), &wdr, perm, {}); rc < 0) {
    return rc;
  }
  if (respect_subvolume_snapshot_visibility &&
...
int Client::get_caps_issued(const char *path, const UserPerm& perms)
{
  ...
  InodeRef in;
  if (int rc = path_walk(get_cwd_ref(), filepath(path), &in, perms, {}); rc < 0) {
    return rc;
  }
  return in->caps_issued();
}
		     ...
		     const UserPerm& perms)
{
  ...
  InodeRef in;
  int r = path_walk(get_cwd_ref(), path, &in, perms, {.mask = CEPH_STAT_CAP_XATTR});
  if (r < 0)
    return r;
  std::scoped_lock lock(*in);
		      ...
		      const UserPerm& perms)
{
  ...
  InodeRef in;
  int r = path_walk(get_cwd_ref(), path, &in, perms, {.followsym = false, .mask = CEPH_STAT_CAP_XATTR});
  if (r < 0)
    return r;
  std::scoped_lock lock(*in);
  return _getxattr(in, name, value, size, perms);
			 ...
			 const UserPerm& perms)
{
  ...
  InodeRef in;
  int r = path_walk(get_cwd_ref(), path, &in, perms, {.followsym = false});
  if (r < 0)
    return r;
  std::scoped_lock lock(*in);
  return _removexattr(in.get(), name, perms);
		     ...
		     size_t size, int flags, const UserPerm& perms)
{
  ...
  InodeRef in;
  int r = path_walk(get_cwd_ref(), path, &in, perms, {});
  if (r < 0)
    return r;
  std::scoped_lock lock(*in);
		     ...
		     const UserPerm& perms)
{
  ...
  vinodeno_t vparent{};
  {
    std::unique_lock parent_lock(*parent);
    vparent = _get_vino(parent);
    ...
    r = _mknod(parent, name, mode, rdev, perms, &in);
  }
  if (r == 0) {
    std::unique_lock in_lock(*in);
    fill_stat(in, attr);
...
int Client::_create(const walk_dentry_result& wdr, int flags, mode_t mode,
		    ...
		    const UserPerm& perms, const std::string& alternate_name,
		    const FSCrypt_Options& fscrypt_options)
{
...
int Client::_symlink(Inode *dir, const char *name, const char *target,
		     const UserPerm& perms, const std::string& alternate_name, InodeRef *inp,
		     const FSCrypt_Options& fscrypt_options)
{
...
#endif
int Client::_rename(Inode *fromdir, const char *fromname, Inode *todir, const char *toname, const UserPerm& perm, const std::string& alternate_name)
{
		      ...
		      const char *newname, const UserPerm& perm)
{
  ...
  std::scoped_lock parent_lock(*parent);
  vinodeno_t vparent = _get_vino(parent);

  std::scoped_lock newparent_lock(*newparent);
...
int Client::ll_fsyncdir(dir_result_t *dirp)
{
  ...
  std::scoped_lock lock(*(dirp->inode));
  ...
  *fhp = NULL;
  ...
  if (r < 0) {
    ...
    if (flags & O_CREAT) {
      ...
      r = _create(wdr, flags, mode, in, fhp, 0, 0, 0, nullptr, &created,
...
bool Client::is_locked() const {
  return m_client_lock.is_locked();
}

bool Client::is_locked_by_me() const {
  return m_client_lock.is_locked_by_me();
}
...
int StandaloneClient::init()
{
  ...
  std::scoped_lock<Client> client_guard(*this);
