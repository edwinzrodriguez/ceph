...
protected:
  void update_in_flight_peak(uint64_t cur,
    ...
    // Peak counters are COUNTER-typed so `perf reset` zeros them. Re-seed
    // the Client-side HWM when that happens so a later interval can record
    // a new peak below the process-lifetime high water mark.
    if (logger->get(peak_counter_key) == 0) {
