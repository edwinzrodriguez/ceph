                                ...
                                const UserPerm& cap_perms)
{
  ...
  in->get_open_ref(in->caps_wanted());
  in->get_cap_ref(cap.issued);
}
...
void ClientCaps::remove_cap(Cap *cap, bool queue_release)
{
  ...
  if (!in.is_any_caps()) {
    ldout(cct, 15) << __func__ << " last one, closing snaprealm " 
                   << in.snaprealm << dendl;
    in.snaprealm_item.remove_myself();
...
void ClientCaps::trim_caps(MetaSession *s, uint64_t max)
{
  ...
  while ((caps_size - trimmed) > max && !p.end()) {
    ...
    if (in->dirty_caps || !in->cap_snaps.empty())
...
int ClientCaps::get_caps(Fh *fh, int need, int want, int *have, loff_t endoff)
{
  ceph_assert(ceph_mutex_is_locked_by_me(client_lock));
  return client->get_caps(fh, need, want, have, endoff);
}

void ClientCaps::kick_flushing_caps(Inode *in, MetaSession *session)
{
  ceph_assert(ceph_mutex_is_locked_by_me(client_lock));
  client->kick_flushing_caps(in, session);
}

void ClientCaps::kick_flushing_caps(MetaSession *session)
{
  ceph_assert(ceph_mutex_is_locked_by_me(client_lock));
  client->kick_flushing_caps(session);
}

void ClientCaps::early_kick_flushing_caps(MetaSession *session)
{
  ceph_assert(ceph_mutex_is_locked_by_me(client_lock));
  client->early_kick_flushing_caps(session);
}

void ClientCaps::flush_caps_sync()
{
  ceph_assert(ceph_mutex_is_locked_by_me(client_lock));
  client->flush_caps_sync();
}
